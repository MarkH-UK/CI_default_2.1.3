...
class Text_formatting {
		...
		else
		{
			...
			else if($my_date >= $yesterday && $my_date < $today)
			{
				if (date("h:i:s", $my_date) == "12:00:00")
				{
					...
					$myOutputDate = 'Yesterday</span>';
				}
				else
				{
                    $myOutputDate = 'Yesterday';
                    if($time === 1)
                    {
                        $myOutputDate .= ' (' . date("g:ia", $my_date) . ')';
                    }
    ...
    {
        $remove = array('at');
        $my_datetime = str_replace($remove, '', $datetime);

        return date('Y-m-d H:i:s', strtotime($my_datetime));
