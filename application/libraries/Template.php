 ...
 *
 * Build your CodeIgniter pages much easier with _partials, breadcrumbs, layouts and themes
 *
 * @package			CodeIgniter
...
class Template
{
	...
	private $_layout_subdir = ''; // Layouts and _partials will exist in views/layouts
	...
	/**
	 * Build the entire HTML output combining _partials, layouts and views.
	 *
	 * @access	public
	 * @param	string
	 * @return	void
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['_partials']	= array();

		// Assign by reference, as all loaded views will need access to _partials
		...
		foreach ($this->_partials as $name => $partial)
		{
			...
			{
				$template['_partials'][$name] = $this->_find_view($partial['view'], $partial['data']);
			}
			...
			else
			{
				...
				$template['_partials'][$name] = $partial['string'];
			}
		}
    ...
    public function inject_partial_array($name, $data = array())
    {
        $this->_partials[$name] = array('data' => $data);
        return $this;
    }
